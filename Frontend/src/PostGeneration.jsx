...
function PostGeneration({ token, onUnauthorized }) {
  // =====================================================
  // DATA STATES
  // =====================================================
  ...
  const [sourceType, setSourceType] = useState("content_plan");
  ...
  async function loadResources() {
    try {
      ...
      const [plans, interviews, knowledge, styles, posts] = await Promise.all([
  ...
  async function scanPrivacy(text) {
    ...
    try {
      ...
      const result = await checkPrivacyGuardrails(token, text);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const result = await generatePost(token, {
        source_type: sourceType,

        source_id: sourceId,

        style_preset_id: selectedStyle || null,

        knowledge_item_ids: selectedKnowledge.length ? selectedKnowledge : null,
      ...
      setPrivacyResult({
        decision: result.privacy_decision,
  ...
  async function handleRegenerate() {
    try {
      ...
      const previousViolations =
        privacyResult?.violations?.map((v) => v.rule_value) || [];

      const result = await regeneratePost(token, {
        source_type: sourceType,

        source_id: sourceId,

        previous_content: generatedPost,

        previous_violations: previousViolations,

        style_preset_id: selectedStyle || null,

        knowledge_item_ids: selectedKnowledge,
      ...
      setMessage("Post regenerated successfully.");
    } catch (error) {
      setError(error.message);
    } finally {
  ...
  async function handleSave() {
    if (!generatedPost.trim()) {
      setError("Generated post is empty.");
    ...
    if (privacyResult?.decision === "block") {
      setError("Blocked post cannot be saved.");
    ...
    try {
      ...
      const saved = await saveGeneratedPost(token, {
        source_type: sourceType,
    ...
    */
      ...
      if (error.status === 400 && error.details) {
        ...
        setError(error.details.message || "Privacy violation detected.");
      } else {
        setError(error.message);
      }
    } finally {
  ...
  async function handleDelete(postId) {
    try {
      await deleteSavedGeneratedPost(token, postId);

      setSavedPosts((current) =>
        current.filter((post) => post.post_id !== postId),
      );
  ...
  return (
    <section className="space-y-6">
      <header>
        <p className="eyebrow">AI Content Creation</p>

        <h1 className="page-title">Post Generation</h1>
...
============================ */}
      ...
      <section className="ui-card p-5 space-y-5">
          ...
          <div>
            ...
            >
              ...
              {(sourceType === "content_plan"
                ...
                <option
                  key={item.content_plan_id || item.interview_id}
                  value={item.content_plan_id || item.interview_id}
                >
                  {item.title || "Voice Interview"}
                </option>
              ))}
            </select>
          </div>
        ...
        <div>
          <label className="field-label">Knowledge Vault</label>
          ...
          <div className="grid md:grid-cols-2 gap-2">
            {knowledgeItems.map((item) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={selectedKnowledge.includes(item.item_id)}
                  onChange={() => toggleKnowledge(item.item_id)}
                />
      ...
      {generatedPost && (
        <section className="ui-card p-5 space-y-5">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <button
              className="btn-primary"
              disabled={saving || privacyResult?.decision === "block"}
              onClick={handleSave}
            >
              <Save />
              Save Post
...
===================== */}
          ...
          <div className="border rounded-lg p-5">
            <h3 className="font-semibold text-lg">Privacy Guardrail Result</h3>

            {checkingPrivacy && <p className="mt-3">Checking privacy...</p>}
              ...
              <div className="mt-4 space-y-4">
                ...
                {privacyResult.violations?.length > 0 && (
                  <div className="space-y-3">
                    <h4 className="font-semibold">Detected Privacy Issues:</h4>

                    {privacyResult.violations.map((v, index) => (
                      <div key={index} className="rounded-lg border p-4">
                        <p>
                          <strong>Rule:</strong> {v.rule_name}
                        </p>

                        <p>
                          <strong>Forbidden Phrase:</strong> {v.rule_value}
                        </p>

                        <p>
                          <strong>Type:</strong> {v.rule_type}
                        </p>
...
============================ */}
      ...
      <section className="ui-card p-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="section-title">Saved Posts History</h2>
        </div>
        ...
        <div className="mt-5 space-y-3">
          ...
          {savedPosts.map((post) => (
            <div key={post.post_id} className="border rounded-lg p-4">
              ...
              <p className="text-xs text-zinc-500 mt-2">{post.platform}</p>
